...
Первой целью - является полный разбор ( свои авторские умозаключения , выводы и вся   остальная 'вода' вроде этой) 
и наидетальнейший конспект почти каждого момента,   воспроизведенным автором видео, коненчо же в моей редакции, 
как ученика, находящегося   на лекции. Если хватит сил и терпения расписывать все 14 уроков на столько подробно, 
как   первый, не исключаю дополнительные отдельные погружения в документацию или другие   ссылки на других авторов 
по разъяснению конкретных моментов. Так же планировал   проанализировать код самой библиотеки, если получится, 
хотя бы на те классы и функции,   которые используются в проекте как минимум.  
...
**models.py** - отвечает за хранение данных в DB  
...
Переходим в **urls.py** , видим *urlpatterns* переменную, являющуюся списком результата работы функции Path. Добавляем новый элемент для наглядности:  
...
def hello(request):
    pass
```
*Надо иметь в виду*, что все функции обработчики, принимают обязательный аргумент request,   который генерит нам браузер.  
...
*Надо иметь в виду*, что в браузере мы переходим по имени blog/, это имя и есть в ранее   указанной переменной urlpatterns:  
```python 
path('blog/', hello)
```
...
Используя джанговый HttpRespone в виде функции, с заголовком, особо много не по-используешь. Для нормальной работы потребуется *html* странички, с которыми будет удобней работать, да и вообще, по-другому никак)  
...
Находим на гитхабе стандартный текст:   [UPD: можно и на бутстрапе сразу]   
...
(я бы раньше подумал, что это внутри функции render надо было бы делать xD)  
Но, нет, джанго умеет делать так: (P.S я на самом деле пока не знаю, но может и не только джанго так умеет)  
```html
<body>
        <h1>Hello world</h1>
        <h2>text</h2>
...
```
Разжевываю что делает джанго. Он начинает цикл в указанном месте по стандартному питонячему циклу, элемент списка *name* внутри списка *names* итерируется. На каждой итерации он "оборачивается" заголовком <p></p>  
...
{% extends 'Blog/base_blog.html' %}  
И после можно скопировать указанные ранее блоки с расширениями из *Blog/base_blog.html*  
...
>>>
```
Вот так эта ссылка с абсолютным путем ведет джанго проект, затем функция Path.joinpath добавляет 'templates', и шаблон становится видимым.   
...
Как выглядит БД на простом примере в excel:
```excel
...
ORM модели ( Object relation mappin)- Это наложение объектов языка Python   
...
ORM нам предоставляет преимущества абстракции. Мы не только данные для БД описываем  
...
*models.SlugField* - это то же самое что и *CharField*, только позволяет использовать  
цифры, буквы в обоих регистрах, нижние подчеркивания и дефисы, все остальные  
...
date_pub = models.DateTimeField(auto_now_add=True)
```
*auto_now_add=True* дата объекта будет создаваться при сохранении в БД.  
...
p.save()
```
Мы сохранили объекты в таблице Post.  
...
атрибутами класса. 
Все операции сохранения, изменения, осуществляется через них.  
...
Что бы посмотреть все объекты, можно вызвать метод *All()*
```bash
>>> Post.objects.all()
...
<Post: New post>
```
iexact - регистро-независимый поиск, который вернет объект из таблицы Post, у   
которого slug = new-slug ( не чувствителен к регистру )
```bash
post = Post.objects.filter(slug__contains='new')
>>> post
<QuerySet [<Post: New post>, <Post: new post2>]>
```
Есть еще метод - filter, который вернет все записи объектов в Post, у которых  
slug = new  
...
```
Меняем старую переменную *n* на новую *posts*, обращаемся к объекту Post, перед  
...
Ищем, где начинается кнопка Home и меняем на этот код, который надо до настроить:  
```html
<li class="nav-item">
...
```
И получается что мы используем url ссылку внутри джанги, которую вставляем в джанговое выражение {% %}
...
который заполнялся в нашей модели постов. К тому же sLug создавался как уникальный.  
...
```
В ней мы создаем переменную, где обращаемся к объекту Post, на подобии как было в Shell, когда создавали сами посты. С помощью метода *slug__iexact* мы обращаемся к регистро-независимому компоненту slug, и его рендерим в еще пока    несозданном *post_detail.html* шаблоне.   
...
{% endblock %}
```
Так же добавляем в *index.html* в строчку с кнопкой READ:  
```html
<a href="{% url 'post_detail_url' slug=post.slug %}" class="btn btn-light">Read</a>
```
В тэг href, указываем джанговый урл.  
Проверяем в браузере.  
Теперь, можно оптимизировать прошлую строчку, так как со временем  
из-за большого кол-ва обьъектов, будет проблематично помнить названия  
ссылок или чего либо другого.  

Перейдем в *models.py*, где создадим специальную функцию, которя будет нам возвращать обсалютную ссылку на конкретный экземпляр класса Post:  

*url reversing, процесс генерации ссылки на какой-то объект*
```Python
def get_absolute_url(self):
    return reverse('post_detail_url', kwargs={'slug': self.slug})
"""помещаем ее между ф-ии Post и __str__"""
```
Теперь дополняем *index.html*:  
```html
...
СОЗДАТЬ КАРТОЧКИ АНКИ ПО 3 УРОКУ - **

### Создание модели Tag, отношения ManyToMany, includes - 4 часть.